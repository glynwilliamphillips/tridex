	...
	for(d_value=0; d_value<=6; d_value++) {
		...
		for (b_value = 6; b_value>= 0; b_value--) {
			...
			for(a_value=0; a_value <= 6; a_value++) {
				...
				temp = gather_minicube_H_TML(HTU_number);
...
function collate_objects_HTML() {
	...
	//console.log('before cube_letter[5]='+cube_letter[5]);
	for ( i = 0; i < I777; i++) {
		mini_cubes[i].HTML_string = gather_minicube_H_TML(i);	
	}
...
sets up: 
cube_H_TML[] the HTML to set up the mini cubes
...
function gather_level_a(temp_a) {
	...
	for (y = 6; y >= 0; y--) {
		for (z = 0; z <=6; z++) {
			make_HTU(temp_a,y,z);
			gather_minicube_H_TML(HTU_number)
			gathered += cube_H_TML[HTU_number];
		}
	}
}
...
save it in cube_H_TML[HTU_number]
*/
 ...
 */
/* used */
function gather_minicube_H_TML(HTU_number) {
//	console.log('gather_minicube_H_TML('+'HTU_number+')');
	cube_H_TML[HTU_number] = '';
	new_line = '';
...
// done on_click
	...
	if (cube_H_TML[HTU_number] == '') {
		cube_H_TML[HTU_number] = new_line;
	} else {
		new_line = cube_H_TML[HTU_number];
	}
	if(started) {
