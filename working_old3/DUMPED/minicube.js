...
get_colour()
set_colour(thiscube, colour)
set_image(thiscube,image name)


This routine has access to all the variables of this Mini_cube to work out what colour it should be
...
function Mini_cube(index_in) {
	...
	this.cube_colour = get_colour(this);
	...
	*/
	...
	coords = '('+this.page_A+','+this.page_B+','+this.page_D+')';
		
	
	new_line = '';
	
	height_D = this.page_D * d_height;
	height_A = this.page_A * a_height;
	height_B = this.page_B * b_height; 
	
	temp = xd*49+xb*7+xa;
	this.display_top = top_displacement   - height_D - height_A - height_B ;
	this.display_left = left_displacement - (xb * b_left) + (xa * a_left);
	console.log('cube_colour ='+this.cube_colour);
	this.image_string = './cube_raw2/cube_raw2_'+this.cube_colour+'.png';
	...
	new_line += '<img id="cube_' +this.HTU_string+'" src="' + this.image_string +'" width="50px" height="50px"';
	new_line += on_click + '>'+'</p>';
...
*/
	...
	/***********************************************/
function set_colour(thiscube, colour) {
	thiscube.cube_colour = colour;
}
	/********************************************/
function set_image(thiscube, image_name) {
}
...
function make_cube_pattern(minicube) {
	...
	new_line += '<img id="cube_' +this.HTU_string+'" src="' + minicube.image_string +'" width="50px" height="50px"';
	new_line += on_click + '>'+'</p>';
