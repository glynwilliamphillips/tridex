...
function answer_selected(word) {
	console.log('answer_selected ('+ word+')');
	chosen = '';
	possible = document.getElementById('answer_list');
	//console.log('possible='+possible);
	//console.log('options = '+possible.options);
...
// debugging nobble 
	...
	for (int_sw = 0; int_sw <= ANSWER_length; int_sw++) {
		...
		//console.log(int_sw+'test '+ this_answer);
		matched = true;
...
//	<option value="AVERAGE" onClick="answer_selected('AVERAGE')">AVERAGE</option>
	 '<option value="' + word + '"' + ' onClick="answer_selected('+"'"+word + "'" +')">'+word+'</option>';	
	 console.log(temp_addition)
	 temp.innerHTML = temp.innerHTML +temp_addition;
}
